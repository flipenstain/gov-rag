...
from query_validator import CypherQueryCorrector, Schema
import json
...
from typing import Optional, Any # Changed 'any' to 'Any' for standard typing
import json
...
class MemgraphQueryToolSpec(BaseToolSpec):
    """
    This class is responsible for querying a Memgraph graph database based on a provided schema definition.
    """

    spec_functions = ["run_request"]

    def __init__(
        self, url, user, password, llm: LLM, validate_cypher: bool = False
    ):
        """
        Initializes the Neo4jSchemaWiseQuery object.

        Args:
            url (str): The connection string for the Memgraph database.
            user (str): Username for the Memgraph database.
            password (str): Password for the Memgraph database.
    ...
    def schema(self):
        return [
            Schema(r["start"], r["type"], r["end"])
            for r in self.structured_schema.get("relationships", [])
        ]
    ...
    def get_system_message(self):
        ...
        """
        return f"""
        Task: Generate Cypher queries to query a Memgraph graph database based on the provided schema definition.
        Instructions:
        ...
        Note: Not yet implemented: atom expression '[relINrelationships(path)|rel.transformation_logic]'
        """

    def query_graph_db(self, memgraph_query, question, params=None):
        """
        Queries the Neo4j database.

        Args:
            memgraph_query (str): The Cypher query to be executed.
        ...
        with self.graph_store.client.session() as session:
            result = session.run(memgraph_query, params)
            output = [r.values() for r in result]
            output.insert(0, list(result.keys()))
            return self.interpret_cypher_result(result, question)
    ...
    def run_request(self, question, history=None, retry=True):
        ...
        print(cypher)
        try:
            return self.query_graph_db(cypher, question)
        # Self-healing flow
    ...
    def interpret_cypher_result(self, result: any, user_question: str) -> str:
        try:
            result_text = json.dumps(result, indent=2, default=str)[:4000]

            prompt = f"""You are a helpful assistant working with a Cypher query system.
...
{result_text}
"""
# Call the LLM to generate the natural language response
            response = self.llm.complete(prompt)
            nl_answer = response.text.strip()
            logger.info(f"Generated natural language answer: {nl_answer}")
            return nl_answer

        except Exception as e:
            logger.error(f"Error interpreting Cypher result: {e}")
            # Fallback: Return raw result or a more informative error
            return f"(Error interpreting results: {str(e)}. Raw results: {str(result)[:500]})"